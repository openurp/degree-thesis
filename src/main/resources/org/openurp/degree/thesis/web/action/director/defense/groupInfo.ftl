   ...
   <div class="card-header">
      ...
      <div style="float:right" >
         ...
         <a href="${b.url('!report?id='+group.id)}" class='btn btn-primary btn-sm'><i class="fa-solid fa-file-excel"></i>下载答辩表</a>
           ...
           [#if group.published]
           <button href="#" class='btn btn-danger btn-sm' disabled="disabled" title="如需删除，请联系秘书取消发布"><i class="fa-solid fa-xmark"></i>已发布，不能删除</button>
           [#else]
           [@b.a href="!remove?defenseGroup.id="+group.id onclick="return bg.Go(this,null,'确认删除该论文答辩分组？');"  class='btn btn-danger btn-sm']<i class="fa-solid fa-xmark"></i>删除[/@]
           [/#if]
